...
import { ArrowLeft, CheckCircle2, CreditCard, Smartphone, Heart } from "lucide-react";
...
export default function DonateProject() {
  ...
  const getCommissionText = () => {
    ...
    return `Проект получит ${received.toFixed(2).replace(/\.00$/, '')} р. с учетом комиссии 0.5%`;
  };
  ...
  if (submitted) {
    return (
      <div className="container mx-auto px-4 py-20 max-w-2xl text-center">
        ...
        <Button asChild size="lg" variant="outline" className="mr-4 rounded-xl">
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-[550px]">
      ...
      <div className="mb-6 text-center">
        <div className="inline-flex items-center justify-center gap-1.5 text-primary font-medium text-xs uppercase tracking-widest mb-3 bg-primary/10 px-3 py-1 rounded-full">
          <Heart className="w-3.5 h-3.5" />
          Целевой сбор
        </div>
        <h2 className="text-xl font-serif font-bold mb-1 text-foreground">
          {project.name}
        </h2>
      </div>

      <div className="bg-white rounded-[2rem] p-6 md:p-10 shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
        
        <h1 className="text-2xl font-serif font-bold text-center mb-8 relative z-10">Сделайте разовое пожертвование</h1>
        
        <Tabs defaultValue="online" className="w-full mb-8 relative z-10">
              ...
              <div className="space-y-4 py-6">
                ...
                <div className="flex items-start space-x-3 group">
                  ...
                  <Label htmlFor="news" className="text-sm font-normal text-slate-500 leading-snug cursor-pointer group-hover:text-slate-700 transition-colors">
                    Хочу получать новости от проекта
                  </Label>
                </div>
              </div>
          ...
          <TabsContent value="bank" className="mt-0">
            <div className="py-16 text-center bg-slate-50 rounded-2xl border border-slate-100 relative z-10">
      ...
      <p className="text-center text-sm text-slate-500 mt-8 px-4">
        Все собранные средства будут зачислены на расчетный счет организации напрямую без скрытых комиссий платформы.
      </p>
    </div>
  );
}
